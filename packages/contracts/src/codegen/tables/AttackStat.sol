...
bytes32 constant _tableId = bytes32(abi.encodePacked(bytes16(""), bytes16("AttackStat")));
bytes32 constant AttackStatTableId = _tableId;

struct AttackStatData {
  uint32 maxDistance;
  uint32 cooldown;
}
...
library AttackStat {
  ...
  /** Get the full data */
  function get(bytes32 key) internal view returns (AttackStatData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32((key));
  ...
  function get(IStore _store, bytes32 key) internal view returns (AttackStatData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32((key));
  ...
  function set(bytes32 key, AttackStatData memory _table) internal {
    set(key, _table.maxDistance, _table.cooldown);
  }

  /** Set the full data using the data struct (using the specified store) */
  function set(IStore _store, bytes32 key, AttackStatData memory _table) internal {
    set(_store, key, _table.maxDistance, _table.cooldown);
  }

  /** Decode the tightly packed blob using this table's schema */
  function decode(bytes memory _blob) internal pure returns (AttackStatData memory _table) {
